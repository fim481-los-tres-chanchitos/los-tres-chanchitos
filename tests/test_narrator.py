...
def test_two_pigs_first_falls() -> None:
    ...
    assert "El **cerdito** corrió hacia la _casa de ladrillos_." in out
    assert (
        "Los" not in out.split("corrieron", maxsplit=1)[0].split("\n")[-1]
        if "corrieron" in out
        else True
    )
    assert "Los **cerditos** corrieron hacia la _casa de ladrillos_." not in out
    assert (
        "Los **cerditos** se quedaron juntos en la _casa de ladrillos_, contentos y a salvo."
        in out
    )
...
def test_two_pigs_both_fall() -> None:
    ...
    assert "La _casa de paja_ cayó." in out
    assert "El **cerdito** corrió hacia la _casa de madera_." in out
    assert (
    ...
    assert "La _casa de madera_ cayó." in out
    assert (
        "Los **cerditos** corrieron hacia la _casa de" not in out
...
def test_three_pigs_two_fall() -> None:
    ...
    assert "El **cerdito** corrió hacia la _casa de madera_." in out
    assert "La _casa de madera_ cayó." in out
    assert "Los **cerditos** corrieron hacia la _casa de ladrillos_." in out
    assert (
        "Los **cerditos** se quedaron juntos en la _casa de ladrillos_, contentos y a salvo."
        in out
    )
